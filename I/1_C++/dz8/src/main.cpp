    ...
    while (answer == 'y'){
        ...
        do {
            ...
            if (current_player == PlayerSign::X) {
                pos = query_player_move_ch(map,size);
            }
			else {
                pos = query_ai_move_ch(map,size);
            }
            assert(set_cell_ch(map, pos.x, pos.y, sign_to_cell(current_player),size));

            outcome = check_turn_outcome_ch(map,size);
            print_field_ch(map,size);
